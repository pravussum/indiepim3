...
public class RRule extends AbstractIcalObject {
  ...
  public String toIcal() {
    ...
    buf.append(":FREQ=").append(getFreq());
     final Weekday _wkst = getWkSt();
     if (null != _wkst) {
      buf.append(";WKST=").append(_wkst.toString());
    }
    if (null != this.until) {
      final DateValue _until = getUntil();
      buf.append(";UNTIL=").append(_until);
      if (_until instanceof TimeValue) {
        buf.append('Z');
      }
    }
    final int count_ = getCount();
    if (count_ != 0) {
      buf.append(";COUNT=").append(count_);
    }
    final int interval_ = getInterval();
    if (interval_ != 0) {
      buf.append(";INTERVAL=").append(interval_);
    }
    final int[] byYearDay_ = getByYearDay();
    if (0 != byYearDay_.length) {
      buf.append(";BYYEARDAY=");
      writeIntList(byYearDay_, buf);
    }
    final int[] byMonth_ = getByMonth();
      if (0 != byMonth_.length) {
      buf.append(";BYMONTH=");
      writeIntList(byMonth_, buf);
    }
    final int[] byMonthDay_ = getByMonthDay();
    if (0 != byMonthDay_.length) {
      buf.append(";BYMONTHDAY=");
      writeIntList(byMonthDay_, buf);
    }
    final int[] byWeekNo_ = getByWeekNo();
    if (0 != byWeekNo_.length) {
      buf.append(";BYWEEKNO=");
      writeIntList(byWeekNo_, buf);
    }
    final List<WeekdayNum> byDay_ = getByDay();
    if (!byDay_.isEmpty()) {
      buf.append(";BYDAY=");
      boolean first = true;
      for (WeekdayNum day : byDay_) {
        if (!first) {
          buf.append(',');
        } else {
          first = false;
    ...
    final int[] byHour_ = getByHour();
    if (0 != byHour_.length) {
      buf.append(";BYHOUR=");
      writeIntList(byHour_, buf);
    }
    final int[] byMinute_ = getByMinute();
    if (0 != byMinute_.length) {
      buf.append(";BYMINUTE=");
      writeIntList(byMinute_, buf);
    }
    final int[] bySecond_ = getBySecond();
    if (0 != bySecond_.length) {
      buf.append(";BYSECOND=");
      writeIntList(bySecond_, buf);
    }
    final int[] bySetPos_ = getBySetPos();
    if (0 != bySetPos_.length) {
      buf.append(";BYSETPOS=");
      writeIntList(bySetPos_, buf);
    }
    return buf.toString();
  }
