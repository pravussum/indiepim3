 ...
 * the License.
 */
package com.google.api.services.gdata;
...
public abstract class GDataClient {
	...
	public final <T> T executeGet(GenericUrl url, Class<T> parseAsType) throws IOException {
		HttpRequest request = getRequestFactory().buildGetRequest(url);
		HttpResponse response = execute(request);
		return response.parseAs(parseAsType);
	}
	...
	protected final <T> T executePatchRelativeToOriginal(GoogleUrl url, AbstractHttpContent patchContent, Class<T> parseAsType, String etag) throws IOException {
		prepareUrl(url, parseAsType);
		HttpRequest request = getRequestFactory().buildRequest(HttpMethod.PATCH, url, patchContent);
		setIfMatch(request, etag);
		return execute(request).parseAs(parseAsType);
	}
