...
public class DeleteMessagesHandler implements Command<DeleteMessages, DeleteMessagesResult> {
    ...
    @Inject
    public DeleteMessagesHandler(MessageUpdateService messageUpdateService, MessageDAO messageDAO, GenericDAO genericDAO, ConnectionUtils connectionUtils) {
    ...
    public DeleteMessagesResult execute(DeleteMessages action) {
        ...
        List<MessagePO> accountMessages = new LinkedList<>();
        ...
        for(final MessagePO curMsg : messages){
            ...
            if(accountId.equals(curMsg.getMessageAccount().getId()) && i < messages.size() -1) {
                ...
                final Set<MessageTagLineageMappingPO> tagLineageMappingsToDelete = new LinkedHashSet<>();
                final Set<Long> messageIdsToDelete = new LinkedHashSet<>();
                // TODO performance tuning
                messageUpdateService.updateImapMessages(userId, messages, accountId, expunge, (folder, imapMessage, messageUID, indieMessage, tagLineageMapping) -> {
                        ...
                        if (account.getDeleteMode().equals(MessageDeleteMethod.MOVE_2_TRASH)) {
                            ...
                            if(!finalDeleteFromTrash) {
                                ...
                                if ((trashFolderTagLineage = account.getTrashFolder()) == null) {
                                    trashFolderTagLineage = connectionUtils.getOrCreateTagLineage(account, trashFolder.getFullName(), trashFolder.getSeparator());
                                }
                ...
                for (Long msgToDelete : messageIdsToDelete) {
                ...
                accountMessages = new LinkedList<>();
			}
			i++;
		}
